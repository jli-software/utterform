...
**What to ask Jonas, and what the log will say.** Which speaker and which microphone — laptop, dock, Bluetooth headset — and whether they are the same device. Then: Settings → Recording → Recording feedback → *Test sounds (5s delay)*, switch to another window, and read the result. All three play → the cue path is fine in the background and the microphone is the culprit; then try Sound → Communications → *Do nothing*, and a wired microphone. None play → the cue path itself is blocked in the background; the log line says whether the stream opened and how long the device took. The log is `%LOCALAPPDATA%\software.jli.utterform\utterform.log`; every real recording writes `microphone … open after`, `start cue played on …` or `start cue not played: …`, and `recording armed … ms after the microphone opened`. For the terminal: which terminal program, and whether a physical Shift+Insert pastes there.
...
Manifests for 0.7.2 are prepared and validated in `packaging/winget/`; see that
