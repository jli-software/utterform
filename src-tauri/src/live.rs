...
use std::{
    collections::HashSet,
    sync::{Arc, Mutex, atomic::{AtomicBool, Ordering}, mpsc as input_queue},
...
use crate::{audio::{self, AudioCaptureState}, domain::{AppSettings, CloudModel, ProcessRequest, ProcessResult, TranscriptionEngine}, feedback::{self, Cue}, history::{self, HistoryEntry}, output, secrets, tray};
...
impl Default for LiveStatus {
    fn default() -> Self {
        Self { text: String::new(), inserted_text: String::new(), delivery_paused: false, warning: None, phase: "connecting" }
    }
}
#[derive(Default)]
...
impl Session {
    fn snapshot(&self) -> LiveStatus { self.status.lock().unwrap_or_else(|e| e.into_inner()).clone() }
    fn update(&self, f: impl FnOnce(&mut LiveStatus)) { f(&mut self.status.lock().unwrap_or_else(|e| e.into_inner())); }
    fn warn(&self, message: &str) {
        self.update(|s| {
            if !s.warning.as_deref().is_some_and(|old| old.contains(message)) {
                s.warning = Some(match &s.warning { Some(old) => format!("{old}\n{message}"), None => message.to_string() });
            }
        });
    }
...
impl LiveState {
    fn session(&self) -> Option<Arc<Session>> { self.0.lock().ok()?.clone() }
    pub fn active(&self) -> bool { self.session().is_some_and(|s| s.active.load(Ordering::Acquire)) }
    pub fn status(&self) -> Option<LiveStatus> { self.session().map(|s| s.snapshot()) }
}
pub fn selected(settings: &AppSettings) -> bool {
    settings.engine == TranscriptionEngine::OpenAi && settings.cloud_model == CloudModel::GptLiveTranscribe
}
#[derive(Serialize)]
pub struct Support { pub supported: bool, pub explanation: &'static str }
pub fn support() -> Support {
    let supported = cfg!(target_os = "windows") || (cfg!(target_os = "linux")
        && std::env::var_os("WAYLAND_DISPLAY").is_some()
        && std::env::var_os("HYPRLAND_INSTANCE_SIGNATURE").is_some());
    Support { supported, explanation: if supported {
    ...
    } else { "Live Dictation in 0.6.0 supports Windows and Omarchy/Hyprland only. Use GPT Transcribe here; macOS Live is planned separately." } }
}

pub fn session_update(settings: &AppSettings) -> Value {
    let mut transcription = json!({"model":"gpt-live-transcribe", "delay":"low"});
    let keywords: Vec<_> = settings.vocabulary.iter().map(|k| k.trim())
        .filter(|k| !k.is_empty() && !k.contains(['<','>','\r','\n'])).collect();
    if !keywords.is_empty() { transcription["keywords"] = json!(keywords); }
    let languages: Vec<_> = settings.language_hints.iter().map(|l| l.trim()).filter(|l| !l.is_empty()).collect();
    if !languages.is_empty() { transcription["languages"] = json!(languages); }
    if !settings.transcription_context.trim().is_empty() { transcription["prompt"] = json!(settings.transcription_context.trim()); }
...
async fn connect(settings: &AppSettings) -> Result<Socket, String> {
    let mut request = "wss://api.openai.com/v1/realtime?intent=transcription".into_client_request()
    ...
    let mut authorization = format!("Bearer {}", secrets::openai_api_key()?).parse::<tokio_tungstenite::tungstenite::http::HeaderValue>()
    ...
    loop {
        let message = socket.next().await.ok_or("OpenAI closed the live session during setup")?
            .map_err(|_| "Live connection failed during setup")?;
        if let Message::Text(text) = message {
            let event: Value = serde_json::from_str(&text).map_err(|_| "Invalid live session response")?;
...
#[derive(Default)]
struct SingleLine { last_cr: bool }
impl SingleLine {
    fn append(&mut self, text: &str) -> String {
        let mut output = String::new();
        for c in text.chars() {
            if c == '\n' && self.last_cr { self.last_cr = false; continue; }
            self.last_cr = c == '\r';
            if matches!(c, '\r'|'\n'|'\t'|'\u{2028}'|'\u{2029}') { output.push(' '); }
            else if !c.is_control() { output.push(c); }
        }
        output
    }
}
...
struct Change { append: String, revised: bool }
impl Transcript {
    fn event(&mut self, event: &Value) -> Result<Change, String> {
        let kind = event["type"].as_str().unwrap_or("");
        let mut change = Change { append: String::new(), revised: false };
        if !matches!(kind, "conversation.item.input_audio_transcription.delta" | "conversation.item.input_audio_transcription.completed" | "conversation.item.input_audio_transcription.failed") { return Ok(change); }
        if let Some(id) = event["event_id"].as_str() {
            if self.events.contains(id) { return Ok(change); }
            if self.events.len() >= MAX_EVENTS { return Err("Live transcript event limit reached".into()); }
            self.events.insert(id.to_string());
        }
        let item = event["item_id"].as_str().ok_or("Live transcription event has no item identity")?;
        match &self.item {
            Some(previous) if previous != item => return Err("Unexpected second audio turn; live delivery stopped to preserve text order".into()),
            None => self.item = Some(item.to_string()),
            _ => {}
        }
        if self.completed { return Ok(change); }
        if kind.ends_with(".failed") { return Err("OpenAI could not transcribe this live audio turn. Received text has been retained.".into()); }
        if kind.ends_with(".delta") {
            let delta = event["delta"].as_str().ok_or("Invalid live transcript delta")?;
            if self.text.len() + delta.len() > MAX_TEXT { return Err("Live transcript size limit reached".into()); }
            self.text.push_str(delta);
            change.append = delta.to_string();
        } else {
            let final_text = event["transcript"].as_str().ok_or("Invalid final live transcript")?;
            if final_text.len() > MAX_TEXT { return Err("Live transcript size limit reached".into()); }
            if let Some(suffix) = final_text.strip_prefix(&self.text) { change.append = suffix.to_string(); }
            else { change.revised = true; }
...
async fn prepare_input(session: Arc<Session>) -> Result<(input_queue::SyncSender<String>, oneshot::Receiver<()>), String> {
    let (sender, receiver) = input_queue::sync_channel::<String>(128);
    let (ready_tx, ready_rx) = oneshot::channel();
    let (done_tx, done_rx) = oneshot::channel();
    std::thread::spawn(move || {
        struct Finished(Arc<Session>);
        impl Drop for Finished { fn drop(&mut self) { self.0.input_finished.store(true, Ordering::Release); } }
        let _finished = Finished(session.clone());
        let mut typer = match crate::typing::LiveTyper::capture() {
            Ok(typer) => { let _ = ready_tx.send(Ok(())); typer }
            Err(error) => { let _ = ready_tx.send(Err(error)); return; }
        };
        loop {
            if session.cancelled.load(Ordering::Acquire) { break; }
            if !session.input_stopped.load(Ordering::Acquire) && let Err(reason) = typer.check_target() {
            ...
            match receiver.recv_timeout(Duration::from_millis(20)) {
                Ok(text) => {
                    if session.input_stopped.load(Ordering::Acquire) { continue; }
                    ...
                    } else { session.update(|s| s.inserted_text.push_str(&text)); }
                }
    ...
    timeout(Duration::from_secs(5), ready_rx).await.map_err(|_| "Live typing initialization timed out")?
        .map_err(|_| "Live typing worker stopped")??;
    Ok((sender, done_rx))
}

pub async fn start(app: AppHandle, settings: AppSettings, input_device: Option<String>) -> Result<(), String> {
    if !support().supported { return Err(support().explanation.into()); }
    if audio::status(&app.state::<AudioCaptureState>())?.recording { return Err("A recording is already active".into()); }
    let session = Arc::new(Session { status: Mutex::new(LiveStatus::default()), active: AtomicBool::new(true),
        cancelled: AtomicBool::new(false), input_stopped: AtomicBool::new(false), input_finished: AtomicBool::new(false), task: Mutex::new(None), settings, started: Instant::now() });
    {
        let state = app.state::<LiveState>();
        let mut slot = state.0.lock().map_err(|_| "Live state unavailable")?;
        if slot.as_ref().is_some_and(|s| s.active.load(Ordering::Acquire)) { return Err("A live recording is already active".into()); }
        *slot = Some(session.clone());
    }
    let setup = async {
        let (input, input_done) = prepare_input(session.clone()).await?;
        let socket = timeout(CONNECT_TIMEOUT, connect(&session.settings)).await.map_err(|_| "Connecting to OpenAI Live timed out")??;
        if session.cancelled.load(Ordering::Acquire) || session.input_stopped.load(Ordering::Acquire) { return Err("The target lost focus during Live setup. Start again from your text field.".into()); }
        let (audio_tx, audio_rx) = mpsc::channel(750);
        let captured_app = app.clone();
        let sound = session.settings.sound_enabled;
        let started = tauri::async_runtime::spawn_blocking(move || {
            audio::start_recording_live(&captured_app.state::<AudioCaptureState>(), input_device.as_deref(), sound, audio_tx)
        }).await.map_err(|_| "Could not open the live microphone")??;
        ...
        std::thread::spawn(move || {
            if started.arm(&cued.state::<AudioCaptureState>()).is_err() {
                let _ = cued.emit("live-cue-warning", "The start sound was unavailable; recording is active.");
            }
        });
        let running_app = app.clone();
        let running_session = session.clone();
        let task = tauri::async_runtime::spawn(async move {
            let outcome = stream(socket, audio_rx, input, &running_app, &running_session).await;
            if let Err(error) = outcome && !running_session.cancelled.load(Ordering::Acquire) {
            ...
            if !matches!(timeout(Duration::from_secs(3), input_done).await, Ok(Ok(()))) {
                running_session.block_input("Live input shutdown timed out; delivery is incomplete.");
            }
            if running_session.snapshot().phase != "failed" {
                running_session.update(|s| s.phase = "completed");
            }
        });
        *session.task.lock().map_err(|_| "Live task state unavailable")? = Some(task);
        ...
        std::thread::spawn(move || {
            loop {
                ...
                match audio::check_limit(&watched.state::<AudioCaptureState>(), recording_id) {
                    ...
                    Ok(audio::LimitCheck::Stopped) => { tray::set_recording(&watched, false); let _ = watched.emit("recording-limit-reached", ()); break; },
                    _ => break,
                }
            }
        });
    ...
    if let Err(ref reason) = setup {
        ...
        session.update(|s| {s.phase = "failed"; s.warning = Some(reason.clone());});
    }
    setup
}

async fn stream(mut socket: Socket, mut audio_rx: mpsc::Receiver<Vec<u8>>, input: input_queue::SyncSender<String>, app: &AppHandle, session: &Session) -> Result<(), String> {
    ...
    loop {
        tokio::select! {
            _ = tick.tick() => {
                ...
                if let Some(error) = audio::live_error(&app.state::<AudioCaptureState>()) { return Err(error); }
...
fn text_batches(text: &str) -> Vec<String> {
    ...
    for (i, c) in text.chars().enumerate() {
        if i > 0 && i % 32 == 0 { batches.push(std::mem::take(&mut buffer)); }
        buffer.push(c);
    }
    if !buffer.is_empty() { batches.push(buffer); }
    batches
}
...
async fn await_input_stop(session: &Session) -> Result<(), String> {
    ...
    while !session.input_finished.load(Ordering::Acquire) {
        if deadline.elapsed() > Duration::from_secs(5) {
            session.block_input("The native input worker did not stop. Restart Utterform before another recording.");
            return Err("Live input shutdown is incomplete. Restart Utterform before another recording.".into());
        }
...
pub async fn cancel(app: &AppHandle) -> Result<(), String> {
    if let Some(session) = app.state::<LiveState>().session()
        ...
        tauri::async_runtime::spawn_blocking(move || audio::cancel_recording(&stopped_app.state::<AudioCaptureState>()))
            .await.map_err(|_| "Could not stop the live microphone")??;
        let task = session.task.lock().map_err(|_| "Live task state unavailable")?.take();
        if let Some(mut task) = task && timeout(Duration::from_secs(7), &mut task).await.is_err() { task.abort(); }
...
pub async fn finish(app: AppHandle, mut request: ProcessRequest) -> Result<ProcessResult, String> {
    let session = app.state::<LiveState>().session().ok_or("No live recording is active")?;
    ...
    let duration_ms = match artifact {
        ...
        Err(error) => { session.warn(&error); session.started.elapsed().as_millis() as u64 }
    };
    let task = session.task.lock().map_err(|_| "Live task state unavailable")?.take();
    if let Some(mut task) = task {
        match timeout(Duration::from_secs(30), &mut task).await {
            Ok(Ok(())) => {},
            _ => { session.cancelled.store(true, Ordering::Release); session.block_input("Live finalization did not complete. Received text has been retained."); task.abort(); }
        }
    }
    await_input_stop(&session).await?;
    session.active.store(false, Ordering::Release);
    ...
    let history_entry = if session.settings.history_enabled && !text.trim().is_empty() {
        ...
        match history::append(&app, entry.clone()) {
            Ok(()) => Some(entry), Err(error) => { warnings.push(format!("History was not saved: {error}")); None }
        }
    } else { None };
    ...
    let (saved_path, copied_to_clipboard) = if !text.is_empty() && (request.copy_to_clipboard || request.save_to_file) {
        match output::deliver(&app, &text, &request, &session.settings) {
            Ok(delivery) => { warnings.extend(delivery.warnings); (delivery.saved_path, delivery.copied_to_clipboard) },
            Err(error) => { warnings.push(error); (None, false) }
        }
    } else { (None, false) };
    if session.settings.sound_enabled && warnings.is_empty() { feedback::play_detached(Cue::Done); }
    Ok(ProcessResult { history_entry, text, saved_path, copied_to_clipboard,
        typed_at_cursor: !status.delivery_paused && !status.inserted_text.is_empty(),
        delivery_warnings: warnings, duration_ms, engine: TranscriptionEngine::OpenAi })
}

#[cfg(test)]
mod tests {
    use super::*;
    fn delta(id: &str, text: &str) -> Value { json!({"type":"conversation.item.input_audio_transcription.delta", "event_id":id,"item_id":"a","delta":text}) }
    fn done(text: &str) -> Value { json!({"type":"conversation.item.input_audio_transcription.completed","item_id":"a","transcript":text}) }
    #[test]
    ...
    fn live_configuration_preserves_hints_without_legacy_language() {
        let settings = AppSettings { vocabulary: vec![" Utterform ".into(), "a\nb".into(), "<bad>".into()], language_hints: vec!["de".into(),"en".into()], transcription_context:"Team meeting".into(), ..Default::default() };
        let value = session_update(&settings);
        let t = &value["session"]["audio"]["input"]["transcription"];
        assert_eq!(t["model"],"gpt-live-transcribe"); assert_eq!(t["keywords"],json!(["Utterform"]));
        assert_eq!(t["languages"],json!(["de","en"])); assert!(t.get("language").is_none());
        assert_eq!(value["session"]["audio"]["input"]["turn_detection"],Value::Null);
    }
    #[test]
    ...
    fn corrections_stay_internal_and_never_backspace() {
        let mut t = Transcript::default(); t.event(&delta("1","Hello there")).unwrap();
        let changed = t.event(&done("Hallo dort!")).unwrap();
        assert!(changed.revised); assert!(changed.append.is_empty()); assert_eq!(t.text,"Hallo dort!");
    }
    #[test]
    fn missing_identity_and_unexpected_turn_fail_closed() {
        let mut t = Transcript::default(); t.event(&delta("1","a")).unwrap();
        let mut e = delta("2","b"); e["item_id"] = json!("b"); assert!(t.event(&e).is_err());
        let e = json!({"type":"conversation.item.input_audio_transcription.delta","delta":"bad"}); assert!(t.event(&e).is_err());
    }
    #[test]
    fn single_line_removes_actions_across_delta_boundaries() {
        ...
        let text = "ä😊".repeat(70); assert_eq!(text_batches(&text).concat(),text);
    }
}
